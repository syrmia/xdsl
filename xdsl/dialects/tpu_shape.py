# proveriti tipove int i float ova tj njihove veliine u ektorima
...
class RotateOp(IRDLOperation):
    ...
    assembly_format = "$value `by` $amount `dim` $dimension (`stride` $stride `stride_dim` $stride_dimension^)? attr-dict `:` type($value) `->` type($result)"
...
class DynamicRotateOp(IRDLOperation):
    ...
    def verify_(self) -> None:
        if self.value.type != self.result.type:
            raise VerifyException(f"{self.name}: value and result must have the same type")
        _verify_rotate_ops(self)
...
def _verify_rotate_ops(op: "RotateOp | DynamicRotateOp") -> None:
    ...
    if op.stride is not None:
        ...
        if stride_val < 0:
            raise VerifyException( f"{op.name}: Rotate stride must be >= 0 if it is specified")
    ...
    if(op.stride is None) != (op.stride_dimension is None):
        raise VerifyException(f"{op.name}: Expected either none or both stride and stride dimensions are present")
...
class IotaOp(IRDLOperation):
    ...
    # koristice generic sintaksu za sad
    def __init__(
        self,
    ...
    def verify_(self) -> None:
        ...
        if in_bw != out_bw:
            if len(in_shape) < 2 or len(out_shape) <2:
                raise VerifyException("tpu.bitcast: Not implemented: bitcast between different bitwidths on a 1D vector.")
...
@irdl_op_definition
class BitcastVregOp(IRDLOperation):
    name = "tpu.bitcast_vreg"
    # tpu_vreg ima predikat any non zero rank vector type pa cemo to modelovati
    input = operand_def(VectorType)
    output = result_def(VectorType)
...
class ScanCountOp(IRDLOperation):
    ...
    traits = traits_def( Pure())
    # druga dva traita cemo obebediti kroz __init__ i verify_
    ...
    def __init__(
        ...
        super().__init__(
            operands=[in_mask, values],
            result_types=[out_mask_type, counts_type]
        )
...
class DynamicGatherOp(IRDLOperation):
    ...
    def verify_(self) -> None:
        ...
        for d in dim_values:
            if d < 0 or d >= rank:
                raise VerifyException(f"tpu.dynamic_gather: Dimensions must be in [0, rank) but got {d}")
            if d in seen:
        ...
        if len(source_shape) != len(output_shape):
            raise VerifyException("tpu.dynamic_gather: Source and result shapes must have the same rank")

        for i in range(len(source_shape)):
            if i not in seen and source_shape[i] != output_shape[i]:
                raise VerifyException("tpu.dynamic_gather: Source and result shpaes must match on non-gather dimensions")
...
class ConcatenateOp(IRDLOperation):
    ...
    assembly_format = "$sources `in` $dimension attr-dict `:` type($sources) `->` type($output)"
    ...
    def verify_(self) -> None:
        if len(self.sources) < 2:
            raise VerifyException("tpu.concatenate: Expected at least 2 operands for concatenate op.")
        ...
        for operand in self.sources:
            ...
            if vty.element_type != first_dtype:
                raise VerifyException("tpu.concatenate: Not implemented: Expected all operands to have the same element type.")
...
class RollVectorsOp(IRDLOperation):
    ...
    def __init__(
        self,
        input_: Sequence[SSAValue | Operation],
        result_type: Attribute
    ):
        super().__init__(
            operands=[list(input_)],
            result_types=[result_type]
        )
